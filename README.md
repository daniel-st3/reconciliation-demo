...
2. **Fetch and Parse CSVs** — Downloads and parses both CSV files using `this.helpers.httpRequest()`
3. **Fuzzy Match** — Runs Jaro-Winkler similarity + amount-diff logic (pure JavaScript, no external libraries)
4. **Rules-Based Summary** — Aggregates counts, identifies top discrepancies, builds flat `sheet_row` object for Sheets logging
5. **Log to Google Sheets** — Appends summary row with 8 columns (active, requires Google Sheets OAuth2 credential)
6. **Respond to Webhook** — Returns JSON with summary counts + top 5 discrepancies to the client
7. **(Optional) LLM Classification** — Disabled by default; sends fuzzy match results to Claude for narrative risk scoring and recommended actions
8. **(Optional) Slack Alert** — Disabled by default; posts summary message to Slack channel when enabled
